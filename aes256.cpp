...
void copyStringToChar(std::string& sInput, int iNum, int iPos, char* pcArray) {
	...
	//if (pcArray == nullptr) throw std::exception("Wrong pointer to array");
...
std::string sAes256Encrypt(std::string& sInput, std::string& sKey){
	...
	//rest
	copyStringToChar(sInput, iNumberOfRest, (iPartCounter - 1) * BLOCK, pcInput[iPartCounter - 1]);

    //add suplements
    for(int i=iNumberOfRest;i<BLOCK;i++){
        pcInput[iPartCounter-1][i]=supplement;
    }
